...
export default function HorizontalProductSlider() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                setLoading(true);
                setError(null);
                const res = await fetch(`${BASE_URL}/api/products/handpicked`, { signal: controller.signal });
    ...
    return (
        <div className="container-fluid px-3 px-md-4 mt-2 mt-md-4 ">
                ...
                {!loading && handpickedProducts.length > 0 && (
                    <div className="horizontal-slider" ref={sliderRef}>
                        {handpickedProducts.map((product) => (
                            <ProductCard specialBadge={true} key={product._id} product={product} col={4} />
                        ))}
                    </div>
                )}
                {!loading && handpickedProducts.length === 0 && (
