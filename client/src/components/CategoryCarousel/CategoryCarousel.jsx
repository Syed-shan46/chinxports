    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                ...
                const apiCats = (mainRes.data?.categories ?? []).filter(c => !c.name || c.name.toLowerCase() !== 'xuping');
                ...
                const merged = categoryDefinitions.map(def => {
                    ...
                    if (subMatch) {
                        ...
                        let mainName = mainId?.name || "";
                        if (mainName.toLowerCase() === 'xuping') {
    ...
    return (
        <section className="pt-12 pb-4 lg:pt-24 lg:pb-12 bg-white overflow-hidden relative">
            <div className="relative z-10">
                ...
                {/* Premium Scrollable Rail */}
                <div
                    ref={scrollRef}
                    className="flex gap-4 lg:gap-8 overflow-x-auto no-scrollbar scroll-smooth snap-x snap-mandatory pr-6 lg:pr-16"
                        ...
                        >
                            <Link to={cat.link} className="flex flex-col items-center w-full">
                                ...
                                <div className="text-center w-full px-1">
                                    <span className="text-[10px] lg:text-[13px] font-bold uppercase tracking-[0.25em] text-charcoal/40 group-hover:text-matte-black transition-colors">
                                        {cat.name}
                                    </span>
                                </div>
                            </Link>
